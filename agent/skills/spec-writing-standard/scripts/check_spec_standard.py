#!/usr/bin/env python3
"""Validate that a full spec follows the standard 18-section structure."""
...
REQUIRED_SUBSECTIONS = [
    ...
    re.compile(r"^### 12\.3 Context Reduction Rules$"),
    ...
    re.compile(r"^### 18\.2 Recommended Extensions$"),
    re.compile(r"^### 18\.3 Operational Validation Before Release$"),
    re.compile(r"^### 18\.4 Spec Update Triggers$"),
]
...
def main() -> int:
    ...
    numbered_headers: list[int] = []
    unexpected_non_numbered: list[str] = []
    ...
    if len(numbered_headers) != len(TOP_LEVEL):
        errors.append(f"expected exactly {len(TOP_LEVEL)} numbered top-level sections, found {len(numbered_headers)}")

    expected_numbers = list(range(1, len(TOP_LEVEL) + 1))
    if numbered_headers != expected_numbers:
        errors.append(f"top-level numbering must be continuous {expected_numbers}, found {numbered_headers}")

    if unexpected_non_numbered:
        errors.append(f"unexpected non-numbered top-level sections present: {unexpected_non_numbered}")
    ...
    print(" - required execution and validation subsections present")
    return 0
