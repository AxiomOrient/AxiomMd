...
- [ ] `intent.md`, `package.yaml`, `requirements.yaml`, `invariants.yaml`, `design.md`, `tasks.md`, `evals.yaml`, `risks.yaml`, `decisions.jsonl`, `contracts/`가 모두 존재한다.
- [ ] `package.yaml`에 `feature_id`, `slug`, `title`, `state`, `review_mode`, `profile_key`, `planes`, `implementation_order`, `owner_roles`, `target_repos`, `adoption`, `current_progress`, `next_step`, `blockers`가 있다.
